    ...
    <table id="table-reporte" class="table table-sm">
        ...
        <tbody>
            ...
            ?>
                ...
                <tr class="c_Sets collapse table-info">
                    <td>Área de WB real por set (pie<sup>2</sup>) TESEO®</td>
                    <?php
                    $totalResult = 0;
                    foreach ($DataSemana as $key => $value) {
                        $result = getRecorrerData($DataSemana[$key]['semanaProduccion'], "areaWBXSetTeseo", $DataSets);
                        $result = $result == '' ? '0' : $result;
                        $totalResult += $result;
