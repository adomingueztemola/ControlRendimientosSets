        ...
        <table class="table table-sm">
            ...
            <tbody>
                ...
                foreach ($DataSublote as $key => $value) {
                    $f_pzasTotales = formatoMil($DataSublote[$key]['pzasTotales'], 2);

                    echo "<tr>
                        <td>{$DataSublote[$key]['loteTemola']}</td>
                        <td>$f_pzasTotales</td>
                        <td><input type='number' name='sublote_{$DataSublote[$key]['id']}' value='{$DataSublote[$key]['unidades']}' 
                        class='form-control sumatoria_sb' id='' min='0' step='0.01' max='{$DataSublote[$key]['pzasTotales']}'></td>
                    </tr>";
...
<script>
    ...
    function validacionPzas(input) {
        ...
        let restante = (piezasRequeridas - piezasTotales).toFixed(2);
