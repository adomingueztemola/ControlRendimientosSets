...
<table class="mt-4 table table-sm">
    <thead>
        <tr>
            ...
            <th>Lote Temola</th>
            <th>Pzas a Vender</th>
            <th>Set's a Vender</th>
            <th>Pzas. Álmacen PT</th>
            <th>Clasif</th>
        </tr>
    </thead>
    <tbody>
        ...
        foreach ($DataVenta as $key => $value) {
            ...
            $suma_sets += $DataVenta[$key]['totalSets'];
            ...
            $colorBtnDist = ($DataVenta[$key]['distribuido'] == "" or $DataVenta[$key]['distribuido'] == "0") ? "btn-danger" : "btn-success";
    ...
    <tfoot>
        <tr class="bg-TWM text-white">
            <td colspan="2"></td>
            <td colspan="1" class="text-center">Total de Unidades: <?= formatoMil($suma_unidades) ?></td>
            <td colspan="1" class="text-center">Total de Set's: <?= formatoMil($suma_sets) ?></td>
...
<script>
    ...
    function seleccionarLote(id, lote, idDetVenta) {
        $("#contentSeleccion").load("../templates/Ventas/cargaSeleccionVenta.php?id=" + id + "&idDetVenta=" + idDetVenta);
