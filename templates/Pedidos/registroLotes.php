        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <select name="pedido" onchange="AreaProveedor()" id="pedido" class="form-control select2" required style="width:100%">
                    ...
                    <?php
                    if ($DataLote[0]['multiMateria'] == '0' or $DataLote[0]['multiMateria'] == '') {
                        $DataPedidos = $obj_pedidos->getPedidosMatPDisp($id);
                    } else {
                        $DataPedidos = $obj_pedidos->getPedidosDispSinMat();
                    }

                    foreach ($DataPedidos as $key => $value) {
                        echo "<option data-tipo='{$DataPedidos[$key]['tipo']}' data-areaprom='{$DataPedidos[$key]['areaWBPromFact']}' data-disponibles='{$DataPedidos[$key]['cuerosXUsar']}' value='{$DataPedidos[$key]['id']}'>
                                     {$DataPedidos[$key]['numFactura']} - {$DataPedidos[$key]['nameProveedor']}  MAT PRIMA: {$DataPedidos[$key]['n_materiaPrima']}</option>";
                    }
                    ?>
                </select>
            </div>
        ...
        <div class="row mt-2">
            <div class="col-md-12">
                <table class="table table-sm">
                    <tbody>
                        <?php if ($DataLote[0]['tipoMateriaPrima'] == 2 or $DataLote[0]['multiMateria'] == '1') { ?>
