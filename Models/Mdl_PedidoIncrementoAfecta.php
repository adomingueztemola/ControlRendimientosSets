...
class Pedido extends ConexionBD
{
    ...
    public function finalizarPedido()
    {
        ...
        $sql = "UPDATE pedidos SET estado='2', cuerosXUsar=totalCuerosFacturados,
                      totalCuerosEntregados=totalCuerosFacturados   WHERE id='$idPedido'";
    ...
    public function registraDetPedido($idRendimiento, $pedido, $areaProv, $_1s, $_2s, $_3s, $_4s, $Total)
    {
        $idUserReg = $this->idUserReg;

        $sql = "INSERT INTO detpedidos (idRendimiento, idPedido, areaProveedorLote, 1s, 2s, 3s, 4s, total_s, idUserReg, fechaReg, estado ) 
        VALUES ('$idRendimiento','$pedido','$areaProv','$_1s', '$_2s', '$_3s', '$_4s', '$Total', '$idUserReg', NOW(), '1')";
    ...
    {
        $idUserReg = $this->idUserReg;
        $sql = "INSERT INTO pedidos(idCatProveedor, 
        numFactura, 
        precioUnitFactPesos,
        precioUnitFactUsd,
        totalCuerosFacturados, 
        areaProvPie2,
        areaWBPromFact,
        idUserReg,
        fechaReg,
        ...
        idPedidoExtra,
        extra, 
        totalCuerosEntregados
        ) 
        SELECT p.idCatProveedor, 
        CONCAT(p.numFactura, '-EX',e.totalCueros), 
        '0.0', 
        '0.0', 
        (p.totalCuerosEntregados+e.totalCueros),
        (p.areaWBPromFact*e.totalCueros),
        p.areaWBPromFact,
        '1',
        NOW(),
        '2',
        ...
        FROM edicionespedidos  e
        INNER JOIN pedidos p ON p.id= e.idPedido
        WHERE e.id='$idEdicion'";
        return  $this->ejecutarQuery($sql, "aumento de Pedido");
    }

    public function aumentaCuerosEntregados($idEdicion){
        $sql="UPDATE pedidos p
              INNER JOIN edicionespedidos e ON e.idPedido= p.id
              SET p.totalCuerosEntregados=(p.totalCuerosEntregados+e.totalCueros), e.cuerosXUsarAnt=(p.totalCuerosEntregados+e.totalCueros)
              WHERE e.id='$idEdicion'";
        return  $this->ejecutarQuery($sql, "aumento total de cueros entregados");
    }
    ...
    public function disminucionPedido($idPedido, $idEdicion)
    {
        ...
        $sql = "UPDATE pedidos p INNER JOIN edicionespedidos e ON e.idPedido= p.id AND e.id='$idEdicion'
                SET e.cuerosXUsarAnt=p.totalCuerosEntregados-e.totalCueros, p.cuerosXUsar= p.cuerosXUsar-e.totalCueros
                WHERE p.id='$idPedido'";
    ...
    public function getPedidosExcepciones($filtradoEstatus = '1=1', $filtradoFecha = "1=1", $filtradoProveedor = "1=1", $filtradoPedidos = "1=1", $filtradoMateria = '1=1')
    {

        $sql = "SELECT p.*, DATE_FORMAT(p.fechaReg,'%d/%m/%Y %H:%i') AS f_fechaReg, 
        DATE_FORMAT(p.fechaFactura,'%d/%m/%Y') AS f_fechaFactura,
        CONCAT(u.nombre,' ',u.apellidos) AS str_usuario,
        cp.nombre AS nameProveedor, 
        cm.nombre AS n_materia, 
				(p.cuerosXUsar+IFNULL(uso.total,0)) AS t_uso
        FROM pedidos p
				LEFT JOIN (SELECT SUM(p.cuerosXUsar) AS total, p.idPedidoExtra FROM pedidos p 
										WHERE p.extra='1' AND p.estado='2'
										GROUP BY p.idPedidoExtra) uso ON uso.idPedidoExtra=p.id
        INNER JOIN segusuarios u ON p.idUserReg=u.id
        INNER JOIN catproveedores cp ON p.idCatProveedor= cp.id
        LEFT JOIN catmateriasprimas cm ON p.idCatMateriaPrima=cm.id
				WHERE (p.extra IS NULL OR p.extra!='1') AND  $filtradoEstatus AND $filtradoFecha AND $filtradoProveedor AND $filtradoPedidos AND $filtradoMateria
        GROUP BY p.id
        ORDER BY p.fechaFactura DESC";
        return  $this->ejecutarQuery($sql, "consultar Pedidos", true);
    }
    ...
    public function getPedidosDisp()
    {

        $sql = "SELECT p.*, cp.nombre AS nameProveedor FROM pedidos p 
                INNER JOIN catproveedores cp ON p.idCatProveedor= cp.id
    ...
    public function getPedidosMatPDisp($idRendimiento)
    {
        ...
        $sql = "SELECT p.*, cp.nombre AS nameProveedor, mp.nombre AS n_materiaPrima FROM pedidos p 
    ...
    {
        $sql = "SELECT dp.id, p.numFactura, dp.total_s, dp.1s, dp.2s, dp.3s, dp.4s, dp.areaProveedorLote FROM detpedidos dp 
