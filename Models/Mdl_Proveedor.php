...
class Proveedor extends ConexionBD
{
    ...
    public function agregarProveedor($name)
    {
        ...
        $sql = "INSERT INTO catproveedores (nombre,estado, idUserReg, fechaReg) 
        VALUES ('{$name}','1','{$idUserReg}',NOW())";
        return $this->ejecutarQuery($sql, "registrar Materia Prima");

    }
    public function getProveedores($filtradoEstatus = '1=1')
    {
        $debug = $this->debug;
        $link = $this->link;
        $ArrayDatos = array();
    ...
    public function getRendimientoAreaXProveedor($filtradoProveedor='1=1'){
       ...
       WHERE $filtradoProveedor AND  r.estado='4' AND r.tipoProceso='1' LIMIT 15";
       //p.idCatProveedor='7'
       return  $this->consultarQuery($sql, "consultar Rendimiento de Área por Proveedor", false);
    }
}
