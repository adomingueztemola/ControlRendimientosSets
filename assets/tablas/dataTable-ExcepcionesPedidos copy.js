    ...
    var dt = $('#table-pedidos').DataTable({
        ...
        "columns": [{
           ...
           
            {
                "data": null,
                render: function(data, type, row) {
                    cuerosXUsar= Intl.NumberFormat('es-MX',{currency:'MXN'}).format(data.t_uso);
                    return cuerosXUsar;
                }
            },
           
            {
                "data": null,
                render: function(data, type, row) {
                    return "<button class='btn btn-xs btn-info' onclick='cargaFormAjuste("+data.id+", \""+data.numFactura+"\")' data-toggle='modal' data-target='#modalExcepcion'><i class='fas fa-people-carry'></i></button>";
                }
            }
