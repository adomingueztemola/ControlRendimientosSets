...
$idUser = $_SESSION['ESMident'];
$nameUser = $_SESSION['ESMnombreUserCto'];
...
class Seguridad
{
	...
	public function Acceso()
	{
		...
		if (isset($_SESSION['ESMident']) and $_SESSION['ESMident'] >= 1 and isset($_SESSION['ESMidNivel']) and $_SESSION['ESMidNivel'] and isset($_SESSION['ESMnombreNivel']) and isset($_SESSION['ESMnombreUser']) >= 1) {
			require('include/connect.php');

			$pag = identificaAccess();

			$this->ident = $_SESSION['ESMident'];
			$this->idNivel = $_SESSION['ESMidNivel'];
			$this->nombreNivel = $_SESSION['ESMnombreNivel'];
			$this->pagina = $pag;
			$this->nombreUser = $_SESSION['ESMnombreUser'];
			$idLevel = $_SESSION['ESMidNivel'];
			$ident = $_SESSION['ESMident'];
			$this->autor = 'RV SETyS';
			$this->lng = 'es';
		} else {
	  ...
	  LIMIT 1";
		...
		if ($cantLnk == 1) {
			...
			$sql = "SELECT CONCAT(em.nombre, ' ', em.apPat) AS fullName, dtlvl.id AS idDetLevel
								FROM segusuarios usr
								INNER JOIN empleados em ON em.id =usr.idEmpleado
		...
		if ($cantPym == 1) {
			...
			return '
					...
					<div class="wsmobileheader clearfix ">
						...
						<span class="smllogo"><img src="assets/menu/images/logo-Esmarla.png" width="80%" alt="" /></span>
					</div>
					<!-- Mobile Header -->
				';
		} else {
	...
	public function customizerDesktop()
	{
		...
		if ($cantPym == 1) {
			...
			return '
					<div class="desktoplogo"><a href="' . $linkLog . '"><img src="assets/menu/images/logo-Esmarla.png" width="80%" alt=""></a></div>
				';
		} else {
			problemas('Se ha Bloqueado por Seguridad, por favor inténtalo de nuevo o notifica a tu Administrador... <br>Error: <b>SEG007</b>');
		}
	...
	public function generaMenuUsuario()
	{
		...
		if ($cantArs >= 1) {
			...
			$iconoImagen = ($_SESSION['ESMgenero'] == 'Masculino') ? 'iconoH.png' : 'iconoM.png';
			return '
	...
	public function generaMenuLateral()
	{
		...
		$sql = "SELECT dtnvl.idMenu, dtnvl.idSubMenu, dtnvl.idSubMenu2 AS menuNivel3,
							mn.nombre AS menu, mn.descripcion AS menuDesc, mn.icono AS menuIco, mn.link AS menuLink, mn.tipo, mn.queryDatos, mn.urlContenido, mn.visible AS mnVisible, mn.usoLinkBase,
							sbmn.nombre AS sbmn, sbmn.descripcion AS sbmnDesc, '' AS subMenuIco, sbmn.link AS sbmnLink, sbmn.visible AS submnVisible,
							sbmn2.nombre AS sbmn2, sbmn2.descripcion AS sbmnDesc2, '' AS subMenuIco2, sbmn2.link AS sbmnLink2, sbmn2.visible AS submn2Visible
            FROM
                segdetnivel dtnvl
						...
						LEFT JOIN segsubmenu2 sbmn2 ON dtnvl.idSubMenu2 = sbmn2.id AND sbmn2.estatus = 1
            WHERE
		...
		while ($dat = mysqli_fetch_array($res)) {
			/*			$mnAct = ($idMenu == $dat['idMenu']) ? 'selected' : '' ;
				...
				$sbMnActN3 = ($idSubMenu2 == $dat['menuNivel3']) ? 'selected' : '' ;
				$sbMnAN3 = ($idSubMenu2 == $dat['menuNivel3']) ? 'active' : '' ;
				...
				$direccion3 = $dat['sbmn2Link'];
				$iconMenu3 = $dat['sbmn2Ico'];*/

			if ($dat['mnVisible'] == '1') {

				if ($dat['submnVisible'] == '0' or $dat['submn2Visible'] == '0') {
					// No entra al Ciclo
				} else {
					...
					if ($mnIdent != $dat['idMenu']) {
						...
						switch ($dat['tipo']) {
							case '1':
								...
								if ($dat['menu'] == 'Inicio') {
									$menuGen .= '<li aria-haspopup="true"><a href="' . $dat['menuLink'] . '" class="' . $mnAct . ' menuhomeicon"><i class="' . $dat['menuIco'] . '"></i><span class="hometext"> ' . $dat['menu'] . '</span></a></li>';
								} else {
									$menuGen .= '<li aria-haspopup="true"><a href="' . $dat['menuLink'] . '" class="' . $mnAct . '"><i class="' . $dat['menuIco'] . '"></i> ' . $dat['menu'] . '</a></li>';
								}
								break;
	...
	public function creaFooter()
	{
		$footer = '
			<footer class="footer text-center">
					...
					<a href="https://rvsetys.com">RV SETyS</a>.
			</footer>
			';
...
function negado($link, $msj)
{
	if ($GLOBALS['devBug'] == 1) {
		...
		$_SESSION['ESMmsjGralPlatform'] = $msj;
